...
class UserSerializer(serializers.ModelSerializer):
    Date_Joind = serializers.SerializerMethodField(method_name='date_join')
    date_joind = serializers.DateTimeField(write_only=True, default=(datetime.now))
...
class UserCartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = ['user_id','menuitem','title','quantity','uint_price','price']
        extra_kwargs = {
            'price':{
                'read_only':True,
            }
...
class OrderItemSerializer(serializers.ModelSerializer):
    unit_price = serializers.DecimalField(max_digits=6, decimal_places=2,source = 'menuotem.price', read_only = True)
    price = serializers.DecimalField(max_digits=6, decimal_places=2, read_only = True)
    title = serializers.CharField(source = 'menuitem.title', read_only = True)
    class Meta:
        model = OrderItem
        fields = ['title', 'quantity', 'unit_price', 'price']
        extra_kwargs = {
            'menuitem':{
                'read_only':True,
            }
...
class UserOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order 
        fields = ['id', 'user', 'delivery_crew', 'status', 'total', 'Date', 'date', 'oreder_items']
        extra_kwargs = {
            'total':{
                'read_only':True,
            }
